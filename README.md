...
Le projet est composé de 3 applications qui partagent une seule base de données MongoDB :
...
| `api-medicapp` | API REST (Node.js/Express) consommée par les deux apps ci-dessus | — |
...
```
                 ...
                 │                                        │
   ┌─────────────┴─────────────┐          ┌───────────────┴───────────────┐
   │  admin-medicapp (3000)     │          │  MedicApp / mobile-web (3001) │
   │  React + nginx (build stat.)│          │  Expo (React Native + web)    │
   │  Back-office interne        │          │  App usagers                 │
   └─────────────────────────────┘          └───────────────────────────────┘
```

4 services Docker (`docker-compose.yml`) : `mongo`, `api`, `admin`, `mobile-web`. `MedicApp` peut aussi tourner en natif (iOS/Android) via Expo, hors Docker, en pointant sur la même API.

## Stack technique

| Service | Techno principale | Détails |
|---|---|---|
| `api-medicapp` | Node.js 20, Express 5, Mongoose 8 | Auth simple (nom + identifiant), upload d'images (Multer), détection d'intention par regex + fallback OpenAI (`gpt-3.5-turbo`), validation Joi |
...
Navigation par onglets en bas d'écran (5 onglets + un écran de détail caché de la barre) :
...
- **Dashboard** (`/`, `/admin`) — statistiques : nombre total de prestataires/utilisateurs, villes couvertes, répartition par ville/catégorie, derniers ajouts, mini-carte de synthèse.
...
| POST | `/api/utilisateurs/login` | "Connexion" par `{ nom, identifiant }` (pas de mot de passe) |
...
Après une modification du code de `MedicApp` ou `admin-medicapp`, il faut reconstruire l'image concernée pour voir le changement (le build web est fait à l'intérieur de l'image Docker, il n'y a pas de hot-reload en mode Docker) :
...
| `REACT_APP_API_BASE_URL` | admin | `http://localhost:5000/api` | Base URL API pour le back-office |
...
- build Docker de chacun des 3 services (sans push),
...
- **Authentification très basique** : le "login" ne vérifie que `nom` + `identifiant` en clair, sans mot de passe ni jeton de session. N'importe qui connaissant le nom et l'identifiant d'un utilisateur peut se connecter à sa place. Il n'y a pas non plus d'authentification sur le back-office admin (`admin-medicapp` est accessible à quiconque a l'URL) ni sur les routes d'écriture de l'API (créer/modifier/supprimer un prestataire ou un utilisateur est ouvert à tous).
- **`helmet` et `express-rate-limit` sont dans les dépendances mais ne sont pas branchés** dans `server.js` — aucune protection anti-bruteforce/DoS basique n'est active malgré leur présence dans `package.json`.
...
- Authentification réelle (mot de passe hashé + session/JWT) côté app cliente **et** protection du back-office admin.
- Activer `helmet` + `express-rate-limit` sur l'API, en particulier sur `/api/utilisateurs/login`.
